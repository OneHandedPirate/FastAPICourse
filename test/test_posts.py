...
def test_create_post_default_published_true(authorized_client, test_user):
    res = authorized_client.post('/posts', json={'title': 'some random title',
                                                 'content': 'some random content'})

    created_post = schemas.PostResponse(**res.json())

    assert res.status_code == 201
    assert created_post.title == 'some random title'
...
def test_unauthorized_user_create_post(client, test_posts):
    res = client.post('/posts', json={'title': 'some random title',
                                      'content': 'some random content'})
...
def test_update_post_non_exist(authorized_client, test_posts, test_user):
    ...
    res = authorized_client.put('/posts/8888888', json=data)
